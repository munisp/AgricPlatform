"""OpenTelemetry wiring for the crop-ml sidecar.
...
- ``OTEL_SERVICE_NAME`` (default ``crop-ml``): ``service.name`` resource attr.
...
instrumentor. ``setup_telemetry`` returns a shutdown callable (flushes and
stops the provider); it is also wired to the FastAPI ``shutdown`` event.
"""
...
logger = logging.getLogger("crop-ml.telemetry")
