 ...
 * (lib/offline-queue.ts), which stays untouched: the queue replays whole
 * API requests, this store syncs individual records.
 *
 * This file is an intentionally duplicated copy of the mobile algorithm
 * (apps/mobile/src/sync/store.ts) — the two apps have separate storage and
 * transport layers and the repo deliberately has no shared client package.
 * Keep behavioural changes in sync between the two copies.
 */
