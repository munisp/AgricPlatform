...
/* --------------------- web storage backend (jsdom) ---------------------- */

describe('web sync storage', () => {
  it('persists the sync state in localStorage across store instances', async () => {
    const { createWebSyncStorage } = await import('../lib/sync/storage');
    window.localStorage.clear();
    const transport = pullTransport([
      page('notification', [{ entityId: 'n-9', version: 3, deleted: false, payload: { title: 'Persisted' } }], 3)
    ]);
    const first = makeStore(transport, createWebSyncStorage());
    await first.pullEntity('notification');

    const reopened = makeStore(pullTransport([page('notification', [], 3)]), createWebSyncStorage());
    await reopened.hydrate();
    expect(reopened.getCursor('notification')).toBe(3);
    expect(reopened.getRecords('notification')[0]?.payload).toEqual({ title: 'Persisted' });
  });
});
